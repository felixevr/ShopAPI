...
namespace Tienda.Application.Services
{
    public class CategoryApplication : ICategoryApplication
    {
        ...
        public async Task<BaseResponse<BaseEntityResponse<CategoryResponseDto>>> ListCategoriesWithProducts()
        {
            var response = new BaseResponse<BaseEntityResponse<CategoryResponseDto>>();
            var categories = await _unitOfWork.Category.ListCategoriesWithProducts();
        ...
        public async Task<BaseResponse<CategorySelectResponseDto>> CategoryById(int categoryId)
        {
            var response = new BaseResponse<CategorySelectResponseDto>();
            var categories = await _unitOfWork.Category.GetByIdAsync(categoryId);

            if (categories is not null)
            {
                response.IsSuccess = true;
                response.Data = _mapper.Map<CategorySelectResponseDto>(categories);
                response.Message = ReplyMessage.MESSAGE_QUERY;
            }
            else
            {
